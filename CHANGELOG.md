...
ClaudeZero now supports two modes to serve both beginners (simple, zero-config) and power users (full control).

- **Turnkey mode (default).** `claude0 init` creates a managed setup with sensible defaults. Policy is locked (header says "managed by claude0"), help text shows only essential commands (`init`, `status`, `expert`, `uninstall`), and the experience is "just works." New `claude0 status` command provides a simple savings summary.
- **Expert mode (opt-in).** `claude0 init --expert` or `claude0 expert` unlocks the policy for editing, shows all advanced commands (`compile`, `doctor`, `policy`, `learn`, `bloat`), and enables full control. Policy header changes to "expert mode — edit freely."
- **Mode tracking.** New `.claude0/mode.json` stores current mode and upgrade timestamp. Backward-compatible: repos without `mode.json` default to turnkey.
- **Mode switching.** `claude0 expert` upgrades (updates `mode.json`, rewrites policy header). `claude0 turnkey` downgrades (locks policy, hides advanced commands).
...
- **M21: reasoning-effort axis.** New `Effort` type + `DEFAULT_EFFORT_BY_TIER`
  ...
  (additive, no schema bump); `claude0 report` shows an effort mix. `test:m21`.
...
- **M23: budget circuit-breaker.** `ZIPLINE_MAX_TOKENS` sets a hard cumulative
  ...
  note (internal `LoopOutcome` `"BUDGET"` is never written); `claude0 report`
...
- **M18: optional gstack integration.** `RepoEnv.gstackInstalled` detects the
  ...
  `claude0 doctor` surfaces an "Orchestration (optional)" section. Detected, never
...
### Added
- **M14: continuous-learning pipeline** — `claude0 learn` mines the ledger and
  ...
  `claude0 learn` prints them; nothing touches `.claude0/rules/` without explicit
...
### Added
- **M13: cross-project policy sync** — `claude0 policy pull|push` makes the
  routing policy a portable, versioned artifact shared across repos. Central
  store is `$ZIPLINE_POLICY_REMOTE` or `~/.claude0/policy.yaml`. `push` publishes
...
### Added
- **M12: terse-output auto-disable, observable in `claude0 doctor`.** New
...
### Added
- **M11: PostToolUse compression of real Bash output.** `claude0 init` now also
  registers a `PostToolUse` hook (matcher `Bash`) calling the new
  `claude0 compress-output` command. It reads the tool-result JSON from stdin,
...
- `uninstall` now strips both claude0 hooks (intercept + compress-output) while
...
### Changed
- `claude0 test` runs milestone tests in simulate mode; the loop's pass/fail is
  now driven by model output, not `Math.random()`.

## [0.2.0] - 2026-07-03

### Fixed
- **Claude Code hook format** — `claude0 init` wrote an invalid hook
  ...
  claude0's own entry, preserving user-added hooks.

### Added
- **Connect-the-pipe (`claude0 intercept`)** — the stub is now a real pipeline:
  ...
  to the ledger. Non-claude0 dirs / failures exit 0 without disturbing the prompt.
...
- **`claude0 doctor`** — shows the integrations stack and per-repo availability
...
- **M1: Context Compiler + Ledger**
  ...
  - Rules system: `.claude0/rules/*.md` with frontmatter tags
...
- **M6: Dashboard**
  - `claude0 report`: runs, savings %, tier mix, escalations
...
- **CLI Commands**
  - `claude0 init [--global]` - Initialize .claude0/ structure
  - `claude0 report [--global]` - Token savings dashboard
  - `claude0 compile "goal" tags` - Manual context compilation
  - `claude0 uninstall [--global] [--force]` - Clean removal with data protection

- **Path Resolution**
  - Upward search for `.claude0/` (like git with `.git/`)
  - Per-repo or global `~/.claude0/` support
